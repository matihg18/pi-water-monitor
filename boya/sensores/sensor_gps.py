...
class SensorGPS(ISensor):
    ...
    def read(self) -> dict:
        if self.serial_conn is None:
            return {"error_gps": "Error Hardware"}
                ...
                try:
                    if line.startswith('$'):
                        ...
                        if isinstance(msg, pynmea2.types.talker.RMC):
                            if msg.status == 'A': 
                                ...
                                if msg.spd_over_grnd:
                                    self.last_data["velocidad_kmh"] = float(msg.spd_over_grnd) * 1.852
                        ...
                        # --- 2. Procesar GPGGA (Necesario para: Altitud, Satélites) ---
                            ...
                            if int(msg.gps_qual) > 0: # 0 = Inválido, 1/2 = GPS Fix
                                self.last_data["latitud"] = float(msg.latitude)
                                self.last_data["longitud"] = float(msg.longitude)
                                self.last_data["gps_status"] = "Fix OK"
                                ...
                                if msg.altitude:
                                    self.last_data["altitud_m"] = float(msg.altitude) 
    ...
    def close(self):
        """Cierra la conexión serial del GPS."""
        if self.serial_conn is not None:
            try:
                self.serial_conn.close()
                print("GPS: conexión serial cerrada.")
            except Exception as e:
                print(f"Error al cerrar conexión GPS: {e}")